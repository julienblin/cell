...
node["cell"].each do |name, values|
  ...
  execute "rebuild npm packages" do
  	command "cd #{"/opt/cell/#{name}/current/src"} && npm rebuild --no-bin-links"
  	timeout 72000
  end
  
  directory "/opt/cell/#{name}/current/src/public/generated" do
    owner "cell-#{name}"
    group "cell-#{name}"
    mode 00755
    action :create
  ...
  template "/etc/init/cell-#{name}.conf" do
    ...
    variables({
      :name => name,
      :src_dir => "/opt/cell/#{name}/current/src",
      :username => "cell-#{name}"
    })
  end
end